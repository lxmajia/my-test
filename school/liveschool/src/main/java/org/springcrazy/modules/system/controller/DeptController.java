...
public class DeptController extends CrazyController {
	...
	public R<List<INode>> list(@ApiIgnore @RequestParam Map<String, Object> dept, CrazyUser bladeUser) {
		QueryWrapper<Dept> queryWrapper = Condition.getQueryWrapper(dept, Dept.class);
		List<Dept> list = deptService.list((!bladeUser.getTenantId().equals(CrazyConstant.ADMIN_TENANT_ID)) ? queryWrapper.lambda().eq(Dept::getTenantId, bladeUser.getTenantId()) : queryWrapper);
	...
	public R<List<DeptVO>> tree(String tenantId, CrazyUser bladeUser) {
		String tenantIdParam = null;
		if(!SecureUtil.isAdministrator()){
			tenantIdParam = Func.toStr(tenantId, bladeUser.getTenantId());
		}
