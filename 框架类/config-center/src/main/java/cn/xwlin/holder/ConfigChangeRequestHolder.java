...
public class ConfigChangeRequestHolder {
  ...
  public void deferredResdultCheck() {
    ...
    while (true) {
      ...
      Iterator<Map.Entry<String, Collection<DeferredResult<String>>>> iterator = myWatchRequests.asMap().entrySet().iterator();
      while (iterator.hasNext()) {
        Map.Entry<String, Collection<DeferredResult<String>>> next = iterator.next();
        String key = next.getKey();
        // 校验是不是都返回了
        Collection<DeferredResult<String>> validDefered = Collections.emptyList();
